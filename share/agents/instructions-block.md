...
- `.github/workflows/release.yml` and `scripts/checkout-dependency.sh` are
  generated by `scripts/adopt.sh` of maelys-release from `adapter/*_PIN`,
  `adapter/PACKAGES` and `packaging/homebrew/*.rb.in`. Never edit them by
  hand; change the declarations, then run `adopt.sh DIR --apply` from a
  maelys-release checkout at the wanted tag. `adopt.sh DIR --check` (exit 2
  on drift) verifies; `adopt.sh DIR --preflight` (exit 3) checks the tag
  preconditions before a release.
