...
| absent | `--mechanism`, `maelys-release` by default | accordingly |

`--mechanism` answers only the third line. On a repository that already
carries a `release.yml`, a `--mechanism` that contradicts it is refused: the
socle never overwrites a workflow it did not generate, and changing the
mechanism of a repository starts by deleting or moving that file.
